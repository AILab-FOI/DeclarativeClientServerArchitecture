...
delete_resource(Req, State) ->
    request:delete(Req,
                   State,
                   fun(Id) -> fakultet_katedra:obrisi_katedru_na_fakultetu(Id) end).
...
run_put_request(#{<<"fakultet">> := Fakultet, <<"katedra">> := Katedra}, Req, State) ->
    request:response(Req,
                     State,
                     fun() -> fakultet_katedra:dodaj_katedru_na_fakultet(Katedra, Fakultet) end);
