...
delete_resource(Req, State) ->
    request:delete(Req, State, fun(Id) -> sadrzaj:obrisi(Id) end).
...
html_request(Req, State) ->
    case utils:gather_html(Req) of
        ...
        {ok, #{id := Id}, Req2} ->
            request:response(Req2, State, fun() -> sadrzaj:dohvati(binary_to_integer(Id)) end);
        {ok, _, Req2} ->
            request:response(Req2, State, fun() -> sadrzaj:dohvati() end)
    end.
...
run_put_request(#{<<"naziv">> := Naziv,
                ...
                Req,
                State) ->
    request:response(Req, State, fun() -> sadrzaj:dodaj(Naziv, Tip, Vrijednost) end);
...
run_patch_request(#{<<"id">> := Id,
                  ...
                  Req,
                  State) ->
    request:response(Req, State, fun() -> sadrzaj:uredi(Id, Naziv, Tip, Vrijednost) end);
