...
allowed_methods(Req, State) ->
    {[<<"GET">>, <<"PATCH">>, <<"OPTIONS">>, <<"POST">>, <<"DELETE">>], Req, State}.
...
delete_resource(Req, State) ->
    request:delete(Req, State, fun(Id) -> sekcija:obrisi(Id) end).
...
html_request(Req, State) ->
    case utils:gather_html(Req) of
        ...
        {ok, #{id := Id}, Req2} ->
            request:response(Req2, State, fun() -> sekcija:dohvati(binary_to_integer(Id)) end);
        {ok, _, Req2} ->
            request:response(Req2, State, fun() -> sekcija:dohvati() end)
    end.
...
run_put_request(#{<<"naziv">> := Naziv, <<"opis">> := Opis}, Req, State) ->
    request:response(Req, State, fun() -> sekcija:dodaj(Naziv, Opis) end);
