...
delete_resource(Req, State) ->
    case utils:gather_json(Req) of
        {ok, Map, Req2} ->
            case student_kolegij:obrisi_studenta_na_kolegiju(
                     maps:get(<<"id_student">>, Map), maps:get(<<"id_kolegij">>, Map))
            of
                {atomic, ok} ->
...
html_request(Req, State) ->
    case utils:gather_html(Req) of
        {error, Reason, _} ->
            request:err(400, Reason, Req, State);
        {ok, #{student := Student, course := Kolegij}, Req2} ->
            request:response(Req2,
                             State,
                             fun() ->
                                student_kolegij:dohvati_studenta_na_kolegiju(binary_to_integer(Student),
...
run_put_request(#{<<"student">> := Student, <<"kolegij">> := Kolegij}, Req, State) ->
    request:response(Req,
                     State,
                     fun() -> student_kolegij:dodaj_studenta_na_kolegij(Student, Kolegij) end);
run_put_request(_, Req, State) ->
    request:err(400, <<"Wrong keys">>, Req, State).

run_patch_request(#{<<"student">> := Id,
                    <<"course">> := Naziv,
                    <<"ocjene">> := Ocjene},
                  Req,
                  State) ->
    request:response(Req, State, fun() -> student_fakultet:uredi(Id, Naziv, Ocjene) end);
